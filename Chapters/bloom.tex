...
\begin{algorithmic}[1]
\Procedure{BloomInsert}{}
\For {$i=0$ to $k-1$}
	\State $j \gets h_i(x) \bmod M$
	\State $bloom[j] \gets 1$
\EndFor
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
quali la collisione e la preimmagine, sebbene ovviamente ci sono scenari applicativi
...
addirittura essere 2. Se $M$ è una potenza di 2, è possibile forzare $h_b(x)$ ad essere sempre
dispari, ma questo riduce l'uniformità della funzione. Per finire, ci saranno alcuni casi in cui
le sequenze di indici tra due elementi potrebbe essere parzialmente sovrapposte sebbene gli hash
siano diversi; si pensi per esempio a $h_b(x_1) = 2h_b(x_2) \bmod M$.

Per risolvere questi problemi, \cite{bloomdoublehash} propone di usare
...
\begin{algorithmic}[1]
\Procedure{BloomUnion}{}
\For {$i=0$ to $m-1$}
	\State $out[i] \gets bloom1[i]$ \textbf{OR} $bloom2[i]$
\EndFor
\State \Return $out$
\EndProcedure
\end{algorithmic}
...
\Procedure{BloomUnion}{}
\For {$i=0$ to $m-1$}
	\State $out[i] \gets bloom1[i]$ \textbf{AND} $bloom2[i]$
\EndFor
\State \Return $out$
\EndProcedure
\end{algorithmic}
...
\begin{itemize}
	...
	$$ M = 1024 \times 1024 \times 8 = 8388608 $$
	$$ N = 2\,000\,000 $$
	$$ P = 0.6185 ^ \frac{M}{N} \approx 0.1463 \approx 15\% $$
	$$ K = \ceil*{-\log_2{P}} = 3 $$

	Quindi possiamo arrivare ad avere circa il 15\% di falsi positivi utilizzando 3 funzioni di
	hash.
\end{itemize}
...
mentre il parametro $s$ può essere usato normalmente con valore $2$, aumentando magari a $4$ se si
prevede un numero molto elevato di elementi.
