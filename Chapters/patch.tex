...
Per soddisfare i requisiti, ci si è orientati sull'implementare i filtri di Bloom scalabili
...
terminando poi con il rilasciare la memoria occupata dalla struttura del filtro stessa \lnnum{5}.
...
di tipo di dato chiamato \ctexttt{OBJ\_BLOOM}. Tutti i tipi di strutture dati hanno una macro con
prefisso \ctexttt{OBJ\_} che si espande ad un progressivo, definite nel file \ctexttt{server.h}.} La
funzione \cverb|checkType| si occupa di fare questo controllo \lnnum{2} e in caso negativo
...
impostati a \cverb|1,1,1| in \lnnum{2}, indicano rispettivamente che: 
...
sesto argomento (impostato a \cverb|NULL| in \lnnum{2}).
...
\begin{figure}
  ...
  \begin{minipage}[c]{0.7\textwidth}
    \subfloat[][Crescita dell'errore (valore configurato: \SI{0.3}{\%})]{%
