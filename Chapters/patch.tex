...
\begin{itemize}
	...
	i filtri di Bloom hanno diversi parametri a disposizione e sono quindi richiesti scelte
	oculate.
\end{itemize}
...
\begin{itemize}
	\medskip
	\item \textbf{BFADD <key> [ERROR p] ELEMENTS <ele1> <ele2> \dots}: aggiunge uno o più elementi al
...
\begin{tabular}{ |l|l|p{210pt}| }
  ...
  \verb|next| & \verb|filter*| & Puntatore al filtro successivo nella catena \\
...
La cancellazione di un filtro avviene tramite una chiamata alla funzione \verb|bloomFilterRelease|,
...
\verb|MurMurHash64A| richiede come abbiamo visto un seme; in questo caso è necessario poter 
...
esso è impostato a zero, si incrementa un contatore chiamato \verb|nbits| che tiene conto di quanti
bit hanno cambiato di stato (da zero ad uno) durante l'inserimento dell'elemento. Questo contatore,
al termine del ciclo, viene sommato al campo \verb|b| del filtro \lnnum{7}, mantenendo così un conto
...
La stima della cardinalità viene implementata dalla funzione qui riportata, \verb|bloomCard|:
...
Analizzando la registrazione di \verb|BFEXIST| \lnnum{2}, si può vedere che il primo campo è il nome
...
Quando il valore del terzo comando è negativo, come per \verb|BFADD| \lnnum{1}, l'analizzatore
