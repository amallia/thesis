...
sulla stessa rete fisica, beneficiando quindi di una latenza molto bassa) perché possano condividere
la stessa cache, aggiornando il codice che prima utilizzava una tabella in RAM dentro il processo
con una libreria che acceda alla stessa tabella tramite chiamate remote su TCP.
...
Per comunicare con Redis, un clienti si deve connettere via TCP (la porta di default è la \num{6379})
...
\begin{description}[style=nextline,font={\bfseries\ttfamily}]
	...
	dell'insieme unione.
	\item[{PFMERGE dest key [key\dots]}] Fonde due o più HyperLogLog, creando un nuovo HyperLogLog
