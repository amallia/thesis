...
\lnnum{1}, come verificato subito dopo con il comando \verb|GET|. 

Il comando \verb|APPEND| \lnnum{2} concatena la stringa \verb|ef| al valore attuale \verb|foo|,
...
in \verb|foo| \lnnum{3}, in particolare i caratteri i cui indici sono compresi nell'intervallo
...
I seguenti comandi consentono infatti la manipolazioni di stringhe rappresentanti numeri in base
10:
...
in base 10 \lnnum{1}; il comando \verb|STRLEN| ci rassicura che si tratta di una stringa di
lunghezza $3$ e non di un numero.

Nonostante questo, il comando \verb|INCR| \lnnum{2} è in grado di manipolare la stringa come
...
Allo stesso modo, il comando \verb|DECRBY| \lnnum{3} sottrae il valore $24$ all'attuale valore
...
\verb|mylist| tramite un comando \verb|RPUSH| \lnnum{1} è sufficiente per creare dinamicamente
...
Se si fosse usato \verb|LPUSH|, l'inserimento in testa avrebbe creato una lista in ordine inverso
rispetto a quello degli argomenti. Le chiamate al comando \verb|LINDEX| nell'esempio confermano
questa struttura.

Il comando \verb|LPOP| \lnnum{2} rimuove il primo elemento dalla testa della lista, restituendolo,
...
Il comando \verb|LINSERT| \lnnum{3} inserisce un nuovo elemento \verb|walt| immediatamente
...
Infine, il comando \verb|LRANGE| \lnnum{4} restituisce una porzione di elementi della lista, cioè
...
\lnnum{1}, creando subito un campo \verb|name| con valore \verb|giovanni|. 
...
Successivamente, tramite \verb|HMSET| \lnnum{2}, vengono aggiunti altri due campi alla tabella:
...
La chiamata a \verb|HMGET| \lnnum{3} restituisce il valore associato al campo \verb|surname|, cioè 
\verb|bajo|. Per quanto riguarda invece le estrazioni multiple di campi e valori, l'esempio
mostra che \verb|HGETALL| restituisce l'intera tabella \lnnum{4}, alternando campi e valori
...
Infinie, \verb|HEXISTS| verifica se il campo \verb|name| è stato definito \lnnum{4}, restituendo $1$
...
Inizialmente, viene creato un insieme tramite \verb|SADD| \lnnum{1}, inserendo $4$ elementi
...
Il test di appartenenza dell'elemento $111$ tramite \verb|SISMEMBER| \lnnum{2} restituisce $1$ per 
indicare che l'elemento è presente. Dopodiché, l'elemento $37$ viene rimosso dall'insieme tramite
\verb|SREM| \lnnum{3}, e i successivi comandi \verb|SCARD| e \verb|ISMEMBER| confermano 
...
e ne calcola rispettivamente l'intersezione tramite \verb|SINTER| \lnnum{5} e la differenza
tramite \verb|SDIFF| \lnnum{6}:
...
Inizialmente, viene aggiunto il valore $50$ all'HyperLogLog denominato \verb|dau| \lnnum{1} (che
...
hash sia dall'indice, e il valore di ritorno \verb|QUEUED| \lnnum{3} \lnnum{4} mostra come
i comandi non sono stati eseguiti immediatamente, ma accodati in attesa della fine della 
definizione della transazione, delimitata dal comando \verb|EXEC| \lnnum{5}.
...
valore corrente \lnnum{1}, e poi \verb|HSET| per inserire il nuovo valore incrementato \lnnum{2}, in
questo modo:
...
In questo caso, dopo aver attivata l'osservazione della chiave con \verb|WATCH| \lnnum{1}, il
...
dal valore restituito da \verb|EXEC| (che è \verb|nil| \lnnum{3}, invece che un array contenente i
