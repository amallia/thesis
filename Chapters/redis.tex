...
La funzione utilizza due parametri principali: una costante moltiplicativa \lnnum{1} e una
costante rotativa \lnnum{2} (in realtà utilizzata per uno shift in questa versione). I valori
...
che non modificano l'hash stesso. Durante il ciclo principale di mix, viene estratta una parola
alla volta dal messaggio (per esempio \SI{32}{\bit}), viene processata in modo irreversibile
tramite due moltiplicazioni ed uno shift con i parametri, e viene infine unita (sempre in
modo irreversibile) all'hash parziale \lnnum{4}. Una volta terminato il messaggio, l'hash stesso
viene finalizzato con dei passaggi irreversibili moltiplicativi e di shift \lnnum{5}, prima di
restituire il valore finale.
...
attività da eseguire; infatti, i comandi \verb|LPOP| e \verb|RPOP| dispongono anche di una versione
...
lista da un altro client. In questo modo, un gruppo di client produttori possono inserire attività
da eseguire (magari codificate da un ID) in una coda di attività pendenti, e un secondo gruppo di
...
delle liste specificate; così facendo è possibile quindi suddividere meglio le attività per priorità
